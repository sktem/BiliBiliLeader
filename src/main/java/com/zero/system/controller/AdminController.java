...
public class AdminController {
    ...
    @ResponseBody
    public AjaxResult deleteAll(String password, HttpSession session){
        ...
        if(!matches){
            ajaxResult.ajaxFalse("密码有误，");
            return ajaxResult;
        }else{
            orderService.delectAll();
            userService.delectAll();
            channelService.delectAll();
            imageService.delectAll();
            invitationCodeService.delectAll();
            userRewardService.delectAll();
            withdrawalService.delectAll();
            ajaxResult.ajaxTrue("清除数据成功!");
            return ajaxResult;
        }
    }
